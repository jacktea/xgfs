...
- `fs.Fs` exposes POSIX-like operations (stat/create/mkdir/remove/list) plus link, copy, and feature flags.
...
## Access Layers
- **CLI (`cmd/xgfs`)**: commands (`ls`, `cat`, `put`, `cp`, `chmod`, `chown`, `rename`, `mkfifo`, `gc`) that now drive the `fs.PosixFs` APIs when available; identity is sourced from the invoking OS user.
...
- **HTTP API (`pkg/server/httpapi`)**: lightweight REST bridging HTTP verbs to fs actions.
- **NFS (`pkg/server/nfs`)**: backed by `github.com/willscott/go-nfs`; exports any `fs.Fs` via a billy adapter, supports API-key/rate limits via shared middleware, and exposes `--export`/`--handle-cache` flags.
- **S3 Gateway (`pkg/server/s3gw`)**: protocol adapter exercised by smoke tests.
  - Supports PUT metadata headers `X-Amz-Meta-Posix-{Mode|Uid|Gid}`, exposes the same headers on GET/HEAD, and accepts `POST /object?rename=/new/path` to delegate `fs.PosixFs.Rename`.
...
- HTTP now supports PATCH/POST to call chmod/chown/rename/mkfifo on any backend implementing `fs.PosixFs`, in addition to paginated directory listings; S3 `list-type=2` honors `max-keys` and emits continuation tokens for smoke tests.
...
- Remote/hybrid `fs.Object` wrappers still need true `ReadAt` support that respects shard caching hints and surfaces partial-read errors instead of panicking.
...
- NFS rename/chmod semantics are limited (directories require re-copying, POSIX owners untouched); extend the billy adapter once core `fs.Fs` grows richer metadata mutations.
